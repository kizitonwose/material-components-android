...
import com.google.android.material.timepickerk.ClockHandView.OnRotateListener;
...
class ClockFaceView extends RadialViewGroup implements OnRotateListener {
  ...
  public ClockFaceView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    LayoutInflater.from(context).inflate(R.layout.kmaterial_clockface_view, this, true);
    clockHandView = findViewById(R.id.material_clock_hand);
    clockHandPadding = res.getDimensionPixelSize(R.dimen.kmaterial_clock_hand_padding);
    ...
    minimumHeight = res.getDimensionPixelSize(R.dimen.kmaterial_time_picker_minimum_screen_height);
    minimumWidth = res.getDimensionPixelSize(R.dimen.kmaterial_time_picker_minimum_screen_width);
    clockSize = res.getDimensionPixelSize(R.dimen.kmaterial_clock_size);
  }
  ...
  private void updateTextViews(@StringRes int contentDescription) {
    ...
    for (int i = 0; i < max(values.length, size); ++i) {
      ...
      if (textView == null) {
        textView = (TextView) inflater.inflate(R.layout.kmaterial_clockface_textview, this, false);
